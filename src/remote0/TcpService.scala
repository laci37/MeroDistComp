...
class TcpService(port: Int, cl: ClassLoader) extends Thread with Service {
      ...
      case None =>
        ...
        try {
          ...
          case uhe: UnknownHostException =>{
            bufferMsg(uhe)
            Debug.info(this+": uhe, buffering")
          }
          case ioe: IOException =>{
            bufferMsg(ioe)
            Debug.info(this+": ioe, buffering")
          }
          case se: SecurityException =>
            Debug.info(this+": secerr")
        }
      case Some(worker) =>
   ...
   * @author Laszlo Mero
   */
  def sendBuffered(n:Node)
  {
	  try {
          val newWorker = connect(n)

          // any pending sends?
          pendingSends.get(n) match {
            case None =>
              // do nothing
            case Some(msgs) =>
              msgs.reverse foreach {newWorker transmit _}
