 ...
 */
private[remote0] class Proxy(node: Node, name: Symbol, @transient var kernel: NetKernel) extends AbstractActor with Serializable {
  import java.io.{ IOException, ObjectOutputStream, ObjectInputStream }

  type Future[+P] = scala.actors.Future[P]

  @transient
  private[remote0] var del: Actor = null
  startDelegate()
...
private[remote0] case class Apply0(rfun: Function2[AbstractActor, Proxy, Unit])

/**
 * @author Philipp Haller
 */
private[remote0] class DelegateActor(creator: Proxy, node: Node, name: Symbol, kernel: NetKernel) extends Actor {
